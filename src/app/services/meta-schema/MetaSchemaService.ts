...
export class MetaSchemaService {
    ...
    public getMetaSchemaFormDic() {
        ...
        const editTableDataSource = metaSchema.editTableDataSource?.map(
            (item) => {
                ...
                if (item?.multiGridSelect?.path) {
                    item.multiGridSelect.imgSrc = getImageProtocolUrl(
                        item.multiGridSelect.path,
                    );
                }
                return item;
            },
    ...
    private createDefaultMetaSchema(filePath: string) {
        const metaSchema: IMetaSchema = {
            ...
            editTableDataSource: [{ id: v4(), randomSeed: 50 }],
        };
        return metaSchema;
    }
    ...
    public saveToFile(metaSchema: IMetaSchema) {
        fs.writeFileSync(
            metaSchema.path,
            JSON.stringify(metaSchema, null, 2),
            'utf-8',
        );
    }
}
