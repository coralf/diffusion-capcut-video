...
export class ArtCreationController {
    ...
    private get globalSettingService() {
        return this.context.getBean<GlobalSettingService>(GlobalSettingService.name);
    }
    ...
    private get stableDiffusionService() {
        return this.context.getBean<StableDiffusionService>(StableDiffusionService.name);
    }
    ...
    private getCleanMetaSchema() {
        const editTableDataSource = this.store.editTableDataSource.map(item => {
    ...
    public handleCellValueChange(changedValue: IEditTableDataItem[keyof IEditTableDataItem], rowData: IEditTableDataItem, dataIndex: keyof IEditTableDataItem) {
        const nextData = this.store.editTableDataSource.slice();
        const changeIndex = nextData.findIndex(item => item.id === rowData.id);
    ...
    public batchHighImage = () => {
        this.stableDiffusionService.textToImage(toJSON(this.store.editTableDataSource), 0, true);
    };


    public batchToImage = () => {
        return this.stableDiffusionService.textToImage(toJSON(this.store.editTableDataSource), 0);
    };
    ...
    public setRowItemLoraPrompts = (loraModels: any[], rowData: any) => {
        const editTableDataSource = this.store.editTableDataSource?.slice() || [];
        const rowIndex = editTableDataSource.findIndex(item => item.id === rowData.id);
        if (rowIndex !== -1) {
    ...
    public copyText = () => {
        const text = this.store.editTableDataSource?.map(item => item.writingText?.trim()).join('\n');
    ...
    public importText = () => {
        navigator.clipboard.readText()
            .then(text => {
                const nextData = this.store.editTableDataSource.slice();
                text.split('\n').forEach(segmentText => {
                    const multiTextInOneSegment = segmentText?.replaceAll(',', '\n').replaceAll('，', '\n').trim();
                    nextData.push({
                        id: v4(),
